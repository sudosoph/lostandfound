...
Posts.deny({
  update: function(userId, post, fieldNames) {
    ...
    return (_.without(fieldNames, 'itemtitle', 'itemdescription', 'locationtags', 'name', 'phone', 'email').length > 0);
  }
});
...
Posts.deny({
  update: function(userId, post, fieldNames, modifier) {
    ...
    return errors.itemtitle || errors.itemdescription || errors.locationtags || errors.name || errors.phone || errors.email;
  }
});
...
validatePost = function (post) {
  ...
  if (!post.itemdescription)
    errors.itemdescription =  "Please fill in an item description";

  if (!post.locationtags)
    errors.locationtags = "Please fill in an item title";
